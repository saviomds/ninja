      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            {profiles.length} registered {profiles.length === 1 ? "user" : "users"}
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800 rounded-xl pl-9 pr-4 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 transition-all"
          />
        </div>
        ...
        <div className="bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden">
          ...
          {loading ? (
            <div className="p-8 text-center text-gray-400 dark:text-gray-500">Loading users…</div>
          ) : filtered.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-800">
              {filtered.map((p) => (
                <div
                  key={p.id}
                  className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-900 dark:text-white text-sm">
                      {p.username || <span className="text-gray-400 italic">No username</span>}
                    </p>
                    <p className="text-xs text-gray-400 dark:text-gray-600 font-mono truncate mt-0.5">{p.id}</p>
                  </div>
