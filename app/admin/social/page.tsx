...
export default function AdminSocialPage() {
  ...
  const inputClass = "w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 transition-all";
    ...
    <div className="min-h-screen bg-[#030712] text-gray-100 p-6 md:p-10">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold text-white">{editId ? "Edit Social Profile" : "Social Links"}</h1>
        </div>

        {/* Quick platforms */}
        <div className="flex gap-2 flex-wrap">
          {COMMON_PLATFORMS.map((p) => (
            <button key={p.name} onClick={() => applyPlatform(p)}
              className="flex items-center gap-2 px-3 py-1.5 bg-gray-900 hover:bg-gray-800 border border-gray-700 hover:border-gray-600 rounded-xl text-xs font-medium text-gray-300 transition-all">
        ...
        <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">{editId ? "Editing profile" : "New social profile"}</h2>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">Platform Name *</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">Icon URL</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">Profile Link *</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">Account Email</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">Account Password</label>
              <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="••••••••" className={inputClass} />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">Followers</label>
            ...
            <div className="flex items-center gap-3 pt-5">
              <button onClick={() => setForm({ ...form, is_active: !form.is_active })}
                className={`relative w-11 h-6 rounded-full transition-colors ${form.is_active ? "bg-cyan-600" : "bg-gray-700"}`}>
            ...
            <div className="sm:col-span-2">
              ...
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} placeholder="Short description of this profile…"
          ...
          <div className="flex gap-3 pt-2">
            ...
            {editId && (
              <button onClick={() => { setForm(emptyForm()); setEditId(null); }}
                className="px-6 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm font-semibold rounded-xl transition-colors border border-gray-700">
                Cancel
              </button>
            )}
          </div>
        ...
        <div className="bg-gray-900/60 border border-gray-800 rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-800">
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">All Profiles ({profiles.length})</h2>
          </div>
          {loading ? (
            <div className="p-8 text-center text-gray-500">Loading…</div>
          ) : profiles.length === 0 ? (
            <div className="p-8 text-center text-gray-500">No social profiles yet.</div>
          ) : (
            <div className="divide-y divide-gray-800">
              {profiles.map((p) => (
                <div key={p.id} className="px-6 py-4 hover:bg-gray-800/30 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-gray-800 border border-gray-700 flex-shrink-0 overflow-hidden flex items-center justify-center">
                      {p.platform_icon ? (
                        <Image src={p.platform_icon} alt={p.platform_name} width={40} height={40} unoptimized className="object-contain w-full h-full p-1" />
                      ) : (
                        <span className="text-lg font-bold text-gray-400">{p.platform_name.charAt(0)}</span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-white text-sm">{p.platform_name}</h3>
                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${p.is_active ? "bg-emerald-500" : "bg-gray-600"}`} />
                      </div>
                      ...
                      {p.followers && p.followers > 0 && <p className="text-xs text-gray-500 mt-0.5">{p.followers.toLocaleString()} followers</p>}
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <button onClick={() => handleToggleActive(p)}
                        className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${p.is_active ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20" : "bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-600"}`}>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 max-w-sm w-full">
            <h3 className="font-bold text-white text-lg mb-2">Delete profile?</h3>
            <p className="text-gray-400 text-sm mb-6">This will remove it from the public-facing site.</p>
