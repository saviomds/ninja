  ...
  const handleSave = async () => {
    ...
    if (editId) {
      ...
      fetch("/api/notify-subscribers", {
        ...
        body: JSON.stringify({ type: "product", name: payload.name, description: payload.description, price: payload.price, category: payload.category, image: payload.image }),
      }).catch(console.error);
    }
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{editId ? "Edit Product" : "Add Product"}</h1>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
            {editId ? "Editing product" : "New product"}
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className={`sm:col-span-2 flex items-center justify-between gap-4 p-4 rounded-xl border transition-colors ${form.is_public ? "bg-emerald-50 dark:bg-emerald-500/5 border-emerald-200 dark:border-emerald-500/20" : "bg-gray-50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700"}`}>
              <div>
                <p className="text-sm font-semibold text-gray-900 dark:text-white">{form.is_public ? "Public" : "Private"}</p>
                <p className="text-xs text-gray-500">{form.is_public ? "Visible to all clients" : "Hidden from clients"}</p>
              </div>
              <button
                type="button"
                onClick={() => setForm({ ...form, is_public: !form.is_public })}
                className={`relative w-12 h-6 rounded-full transition-colors flex-shrink-0 ${form.is_public ? "bg-emerald-500" : "bg-gray-300 dark:bg-gray-600"}`}
              >
          ...
          {form.image && (
            <div className="mt-2">
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Image Preview</p>
              <div className="w-24 h-24 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                <Image src={form.image} alt="preview" width={96} height={96} unoptimized className="object-cover w-full h-full"
        ...
        <div className="bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400 dark:text-gray-500">No products yet. Add one above.</div>
          ) : (
            <div className="divide-y divide-gray-50 dark:divide-gray-800">
              {products.map((p) => (
                <div key={p.id} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                  <div className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex-shrink-0 overflow-hidden">
                    {p.image ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                      <span className="text-xs text-cyan-600 dark:text-cyan-400 font-semibold">Rs {p.price.toLocaleString()}</span>
                      {p.category && <span className="text-xs text-gray-500">{p.category}</span>}
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-md ${p.stock === 0 ? "bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400" : p.stock <= 5 ? "bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400" : "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400"}`}>
                        {p.stock === 0 ? "Out of stock" : `${p.stock} in stock`}
                      </span>
                      <button
                        onClick={() => handleTogglePublic(p)}
                        className={`text-xs font-medium px-2 py-0.5 rounded-md border transition-colors ${(p.is_public ?? true) ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20 hover:bg-emerald-100 dark:hover:bg-emerald-500/20" : "bg-gray-100 dark:bg-gray-800 text-gray-500 border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-500 hover:text-gray-600 dark:hover:text-gray-400"}`}
                      >
...
function Field({ label, value, onChange, type = "text", placeholder = "", className = "", as = "input", options = [] }: {
  ...
  const base = "w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 transition-all";
  return (
