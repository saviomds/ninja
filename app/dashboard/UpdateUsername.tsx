...
export default function UpdateUsername({ userId, currentUsername, onUpdate }: { userId: string, currentUsername: string | null, onUpdate: () => void }) {
  ...
  return (
    <form onSubmit={handleUpdate} className="flex flex-col gap-4 max-w-sm bg-white dark:bg-gray-900/40 backdrop-blur-md border border-gray-200 dark:border-gray-800 rounded-2xl p-6">
      <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Set your Username</label>
      <input
        type="text"
        value={username}
        onChange={(e) => setUsername(e.target.value)}
        placeholder="e.g. tech_ninja"
        className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2 text-base md:text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-indigo-500 transition-colors"
        required
      />
