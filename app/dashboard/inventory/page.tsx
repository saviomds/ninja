    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white">
      ...
      <div className="sticky top-0 z-30 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 px-4 md:px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-gray-900 dark:text-white">Inventory Management</h1>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setViewMode(v => v === "grid" ? "table" : "grid")}
            className="px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-600 dark:text-gray-300 transition-all">
      ...
      <div className="max-w-screen-xl mx-auto px-4 md:px-6 py-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search items..."
            className="flex-1 min-w-[180px] bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-cyan-500 transition-all" />
            ...
            {categories.map(c => (
              <button key={c} onClick={() => setCategoryFilter(c)}
                className={`px-3 py-1 text-xs rounded-full border font-medium transition-all ${
                  categoryFilter === c ? "bg-cyan-500/20 text-cyan-400 border-cyan-500/50" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500"
                }`}>
                {c === "all" ? "All" : c}
              </button>
            ))}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          {item.image ? (
                            <img src={item.image} alt={item.name} className="w-10 h-10 rounded-lg object-cover bg-gray-100 dark:bg-gray-800" />
                          ) : (
                            <div className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-400 dark:text-gray-600 text-lg">📦</div>
                          )}
                          <div>
                      ...
                      <td className="px-4 py-3">
                        {item.category ? (
                          <span className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-full">{item.category}</span>
                        ) : <span className="text-gray-600">—</span>}
                      </td>
                      <td className="px-4 py-3 font-semibold text-gray-900 dark:text-white">Rs {item.price.toLocaleString()}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => { setAdjustItem(item); setAdjustAmount("-1"); setIsAdjustOpen(true); }}
                            disabled={adjustingId === item.id} className="w-6 h-6 rounded-md bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white text-sm flex items-center justify-center transition-all disabled:opacity-50">−</button>
                          <span className="text-gray-900 dark:text-white font-semibold w-8 text-center">{item.stock}</span>
                          <button onClick={() => { setAdjustItem(item); setAdjustAmount("+1"); setIsAdjustOpen(true); }}
                            disabled={adjustingId === item.id} className="w-6 h-6 rounded-md bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white text-sm flex items-center justify-center transition-all disabled:opacity-50">+</button>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${item.is_public ? "text-emerald-400 bg-emerald-500/10" : "text-gray-500 bg-gray-100 dark:bg-gray-800"}`}>
        ...
        {viewMode === "grid" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            {filtered.map(item => {
              const s = getStockStatus(item.stock);
              return (
                <div key={item.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden hover:border-gray-300 dark:hover:border-gray-700 transition-all">
                  <div className="h-40 bg-gray-100 dark:bg-gray-800 relative">
                    {item.image ? (
                      <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-4xl text-gray-400 dark:text-gray-700">📦</div>
                    )}
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex justify-between items-baseline mt-2">
                      <span className="text-lg font-bold text-gray-900 dark:text-white">Rs {item.price.toLocaleString()}</span>
                      <span className="text-xs text-gray-400">qty: {item.stock}</span>
                    </div>
                    <div className="flex gap-2 mt-3">
                      <button onClick={() => { setAdjustItem(item); setAdjustAmount(""); setIsAdjustOpen(true); }}
                        className="flex-1 py-1.5 text-xs bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-lg transition-all">Stock</button>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm overflow-y-auto p-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl my-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
              <h2 className="font-bold text-lg text-gray-900 dark:text-white">{editingId ? "Edit Item" : "Add Inventory Item"}</h2>
              <button onClick={() => setIsFormOpen(false)} className="text-gray-500 hover:text-gray-900 dark:hover:text-white text-xl transition-colors">✕</button>
            </div>
            <div className="px-6 py-5 space-y-3 max-h-[calc(100vh-220px)] overflow-y-auto">
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                <span className="text-sm text-gray-600 dark:text-gray-300">Visible to customers</span>
              </div>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-800">
              <button onClick={() => setIsFormOpen(false)} className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg hover:text-gray-900 dark:hover:text-white transition-all">Cancel</button>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl w-full max-w-sm shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
              <div>
                ...
                <h2 className="font-bold text-gray-900 dark:text-white">{adjustItem.name}</h2>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-500">Current</p>
                <p className="text-2xl font-black text-gray-900 dark:text-white">{adjustItem.stock}</p>
              </div>
            </div>
            <div className="px-6 py-5 space-y-3">
              ...
              {adjustAmount && !isNaN(parseInt(adjustAmount)) && (
                <div className="bg-gray-100 dark:bg-gray-800 rounded-xl p-3 text-sm">
                  New stock: <span className="font-bold text-gray-900 dark:text-white">{Math.max(0, adjustItem.stock + parseInt(adjustAmount))}</span>
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-800">
              <button onClick={() => { setIsAdjustOpen(false); setAdjustItem(null); }} className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg hover:text-gray-900 dark:hover:text-white transition-all">Cancel</button>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            <p className="text-gray-900 dark:text-white font-semibold mb-2">Delete this item?</p>
