    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white">
      ...
      <div className="sticky top-0 z-30 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 px-4 md:px-6 py-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h1 className="font-bold text-gray-900 dark:text-white truncate">Repair Tickets</h1>
          <span className="flex-shrink-0 text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 px-2 py-0.5 rounded-full">{tickets.length}</span>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={() => setViewMode(v => v === "pipeline" ? "list" : "pipeline")}
            className="px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-600 dark:text-gray-300 transition-all hidden sm:block">
          ...
          {(["pipeline","list"] as const).map(v => (
            <button key={v} onClick={() => setViewMode(v)}
              className={`flex-1 py-1.5 text-xs font-semibold rounded-lg border transition-all capitalize ${
                viewMode === v ? "bg-cyan-500/20 border-cyan-500/40 text-cyan-300" : "bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400"
              }`}>{v}</button>
          ))}
        </div>
              ...
              {PIPELINE.map(stage => {
                ...
                return (
                  <div key={stage.key} className="w-60 flex-shrink-0">
                    ...
                    <div className="bg-gray-100/80 dark:bg-gray-900/50 border border-t-0 border-gray-200 dark:border-gray-800 rounded-b-xl min-h-[160px] p-2 space-y-2">
                      {cols.length === 0 && (
                        <p className="text-xs text-gray-700 text-center py-8">Empty</p>
                      )}
                      {cols.map(ticket => (
                        <div key={ticket.id} onClick={() => openDetail(ticket)}
                          className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 rounded-lg p-3 cursor-pointer transition-all hover:shadow-lg">
                          ...
                          <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{ticket.customer_name}</p>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map(ticket => {
                      ...
                      return (
                        <tr key={ticket.id} className="border-b border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <p className="text-gray-900 dark:text-white font-medium whitespace-nowrap">{ticket.customer_name}</p>
                            <p className="text-xs text-gray-500">{ticket.customer_phone || "—"}</p>
                          </td>
                          <td className="px-4 py-3">
                            <p className="text-gray-900 dark:text-white whitespace-nowrap">{ticket.device_brand} {ticket.device_model}</p>
                          ...
                          <td className="px-4 py-3 text-gray-600 dark:text-gray-300 whitespace-nowrap">{ticket.technician || "—"}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 backdrop-blur-sm overflow-y-auto p-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl w-full max-w-2xl my-4 shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
              ...
              <button onClick={() => setIsFormOpen(false)} className="text-gray-500 hover:text-gray-900 dark:hover:text-white text-xl transition-colors leading-none">✕</button>
            </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <div>
                    ...
                    <select value={form.priority} onChange={e => setField("priority")(e.target.value)}
                      className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-cyan-500 transition-all">
                      <option value="low">Low</option>
                      <option value="normal">Normal</option>
                      <option value="high">High</option>
                      <option value="urgent">Urgent</option>
      ...
      {isDetailOpen && selectedTicket && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 backdrop-blur-sm overflow-y-auto p-4" onClick={() => setIsDetailOpen(false)}>
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl my-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {/* Header */}
            <div className="flex items-start justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 gap-4">
            ...
            <div className="px-6 py-5 space-y-5 max-h-[calc(100vh-200px)] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900 dark:text-white">{new Date(selectedTicket.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</p>
                </div>
                ...
                {selectedTicket.estimated_completion && (
                  <div>
                    ...
                    <p className="text-sm text-gray-900 dark:text-white">{new Date(selectedTicket.estimated_completion).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-800 rounded-lg px-3 py-2 leading-relaxed">{selectedTicket.issue_description}</p>
              </div>

              {selectedTicket.notes && (
                <div>
                  <p className="text-xs text-gray-500 mb-1">Internal Notes</p>
                  <p className="text-sm text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 rounded-lg px-3 py-2 leading-relaxed">{selectedTicket.notes}</p>
                </div>
              )}
                    ...
                    <button key={stage.key}
                      ...
                      className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all ${
                        selectedTicket.status === stage.key
                          ...
                          : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500 hover:text-gray-700 dark:hover:text-gray-200"
                      }`}
                    >
                      {stage.label}
                    </button>
                      ...
                      return (
                        <div key={h.id} className="flex items-start gap-3 text-xs">
                          <div className="relative flex flex-col items-center flex-shrink-0">
                            ...
                            {i < statusHistory.length - 1 && <div className="w-px flex-1 bg-gray-200 dark:bg-gray-800 mt-1" style={{ minHeight: "16px" }} />}
                          </div>
            ...
            <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-800">
              ...
              <div className="flex gap-2">
                <button onClick={() => openEdit(selectedTicket)} className="px-4 py-2 text-sm bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-900 dark:text-white rounded-lg transition-all">Edit</button>
              ...
              return (
                <>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-bold text-gray-900 dark:text-white">Move to <span className={stage.color}>{stage.label}</span></h3>
                  </div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    autoFocus
                    className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-cyan-500 transition-all resize-none mb-4"
                  />
                  <div className="flex justify-end gap-3">
                    <button onClick={() => setPendingStatus(null)} className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg hover:text-gray-900 dark:hover:text-white transition-all">Cancel</button>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setDeleteId(null)}>
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 max-w-sm w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            <p className="font-semibold text-gray-900 dark:text-white mb-2">Delete this ticket?</p>
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-5">This cannot be undone. All status history will be removed.</p>
            <div className="flex justify-end gap-3">
